...
Redux는 작은 라이브러리입니다. 하지만 Redux의 규약과 API는 도구와 확장 생태계를 만들기 위해 신중하게 선택되었으며, 커뮤니티에서 다양한 애드온, 라이브러리, 도구를 만들어내고 있습니다. Redux를 사용하기 위해 이러한 애드온을 사용해야만 하는 것은 아니지만, 이들이 앱의 기능을 구현하거나 문제를 해결하기 쉽게 만들어줄 것입니다.

Redux와 관련된 라이브러리, 애드온, 도구의 방대한 목록을 보고 싶다면 [Redux Ecosystem Links](https://github.com/markerikson/redux-ecosystem-links)를 확인하세요. [React/Redux Links](https://github.com/markerikson/react-redux-links) 또한 React나 Redux를 배우려는 이들을 위한 강좌와 유용한 리소스들을 포함하고 있습니다.

이 페이지는 Redux 메인테이너들이 개인적으로 조사했거나 커뮤니티 내에서 널리 쓰이는 애드온들을 나열합니다. 이 목록에 들어있지 않더라도 시도해보는 것을 주저하지는 마세요. 생태계가 너무 빠르게 성장하고 있기 때문에, 저희도 전부 찾아보기에는 시간이 부족했습니다. 아래 목록은 '개발팀이 추천한' 것들 정도로만 받아들여주세요. Redux와 함께 사용할 수 있는 멋진 도구가 있다면 망설이지 말고 풀 리퀘스트를 보내주세요.
